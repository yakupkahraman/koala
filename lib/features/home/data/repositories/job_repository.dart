...
import 'package:koala/features/company_detail/data/company_repository.dart';
...
class JobRepository {
  ...
  /// Fake iş verilerini companyDetails ile birlikte döndür
  Future<List<JobModel>> _getJobsWithCompanyDetails() async {
    final jobs = await _getFakeJobs();
    final jobsWithCompany = <JobModel>[];

    for (var job in jobs) {
      final company = await _companyRepository.getCompanyByName(job.company!);
      jobsWithCompany.add(
        JobModel(
          id: job.id,
  ...
  Future<List<JobModel>> _getFakeJobs() async {
    return [
      JobModel(
        id: '1',
        latitude: 39.9334,
      ...
      JobModel(
        ...
        title: 'Kedi Bakıcısı',
        subtitle:
            '2 haftalık tatil süresince 2 kediye bakacak güvenilir birisi.',
        price: 1200,
      ...
      JobModel(
        ...
        subtitle:
            'Specialty coffee deneyimi olan barista aranıyor. Tam zamanlı.',
        price: 15000,
  ...
  Future<List<JobModel>> getJobs() async {
    ...
    return _getJobsWithCompanyDetails();
  }
  ...
  Future<JobModel?> getJobById(String id) async {
    ...
    try {
      final jobs = await _getJobsWithCompanyDetails();
